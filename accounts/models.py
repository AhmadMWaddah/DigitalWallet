...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    is_verified = models.BooleanField(
        "Email Verified", default=False, help_text="Indicates if the email has been verified."
    )
    ...
    date_joined = models.DateTimeField(
        "Date Joined", default=timezone.now, help_text="The date and time when the user joined."
    )
...
class StaffProfile(models.Model):
    ...
    assigned_permissions = models.TextField(
        "Assigned Permissions", blank=True, help_text="Comma-separated list of special permissions."
    )

    created_at = models.DateTimeField(
        "Created At", auto_now_add=True, help_text="When this profile was created."
    )

    updated_at = models.DateTimeField(
        "Updated At", auto_now=True, help_text="When this profile was last updated."
    )
...
class ClientProfile(models.Model):
    ...
    full_name = models.CharField(
        "Full Name", max_length=255, blank=True, help_text="User's full legal name."
    )

    company = models.CharField(
        "Company", max_length=255, blank=True, help_text="Company or organization name."
    )

    job_title = models.CharField(
        "Job Title", max_length=255, blank=True, help_text="Current job title or occupation."
    )

    phone = models.CharField(
        "Phone Number", max_length=20, blank=True, help_text="Contact phone number."
    )

    address = models.TextField("Address", blank=True, help_text="Physical address.")
